...
<body>
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
                <?php
                  $rows = $db->query("SELECT i.* FROM snippets i LEFT JOIN snippets j ON i.userid = j.userid AND i.sid < j.sid WHERE j.userid is NULL ORDER BY sdate DESC;");
                  foreach ($rows as $row) {
                    $temp_userid = $row["userid"];
                    $temp_db = $db->query("SELECT name FROM students WHERE students.id = '$temp_userid';");
                    $temp_username = $temp_db->fetchColumn();
                    ...
                    <div class="post-preview">
                        ...
                        <p class="post-meta">Posted by <a href="snippets.php?userid=<?=$temp_userid?>"><?=$temp_username?></a> on <?=$temp_date ?></p>
                    </div>
                    <hr>
                    <?php
                  }
